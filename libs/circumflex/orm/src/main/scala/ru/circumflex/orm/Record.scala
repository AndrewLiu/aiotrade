 ...
 * </ul>
 * @param table explicitly feed companion object table to Record for <code>val relation</code>. by Caoyuan
 */
abstract class Record[R](val relation: Relation[R]) {
  ...
  /** by Caoyuan */
//  private def getRelation: Relation[R] = {
//    val clazz = getClass
//    val compainObjectClassConstructor =
//      Class.forName(clazz.getName + "$", true, Circumflex.classLoader).getDeclaredConstructor()
//    compainObjectClassConstructor.setAccessible(true)
//    val relation = compainObjectClassConstructor.newInstance().asInstanceOf[Relation[R]]
//    relation
//  }
