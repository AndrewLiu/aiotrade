...
object PinYin {
  ...
  private val spellMap = new LinkedHashMap[String,String](20901)

  initialize


  private def spellPut(ascii: String, spells: String) {
  ...
  def getCnSpell(cn: Char): String = {   
    ...
    try {
      ...
      if (bytes.length < 2) {
        ...
        val hiByte = 256 + bytes(0) // int
        val loByte = 256 + bytes(1) // int
        ascii = (new StringBuilder(String.valueOf(hiByte))).append("-").append(loByte).toString
        val spell = spellMap.get(ascii)
        if (spell == null) {
          return String.valueOf(cn)
        } else spell
      }
    } catch {
      case ex: UnsupportedEncodingException => String.valueOf(cn)
    }
  }
	

  def getFullSpell(cnStr: String): String = {
  ...
  def getFirstSpell(cnStr$: String): String = {
   val cnStr = toDBC(cnStr$)
   if (cnStr == null || "".equals(cnStr.trim))
     return cnStr
    ...
    while (i < chars.length) {
      ...
      if (spell.length() < 1) {
    	...
    	val FirstLetter = spell.substring(0, 1)
    	sb.append(FirstLetter)
      }
      i += 1
    }
	
    sb.toString().replace(" ", "").toUpperCase
  }
  ...
  def main(args: Array[String]) {
    val test = getCnSpell('鄂')
    println(test)
		
    val t1 = System.currentTimeMillis
  
    val ss = Array("This is 俄国", 
                   "上证指数",
                   "中国石油",
                   "ＳＴ＊东海",
                   "国债（１）",
