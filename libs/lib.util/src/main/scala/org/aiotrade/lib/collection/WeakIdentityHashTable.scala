 ...
 */
trait WeakIdentityHashTable[K, V] {
  import WeakIdentityHashTable._
  ...
  protected def elemEquals(key1: K, key2: K): Boolean = {
    m.toString match {
      case "Byte" | "Short" | "Char" | "Int" | "Long" | "Float" | "Double" => key1 == key2
      case _ => key1.asInstanceOf[AnyRef] eq key2.asInstanceOf[AnyRef]
    }
  }

  protected def elemHashCode(key: K) = if (key == null) 0 else hash(key)
...
private object WeakIdentityHashTable {
