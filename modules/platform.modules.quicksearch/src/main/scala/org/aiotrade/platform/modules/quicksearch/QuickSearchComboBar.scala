...
package org.aiotrade.platform.modules.quicksearch;
...
class QuickSearchComboBar(ks: KeyStroke) extends AbstractQuickSearchComboBar(ks) {
  ...
  val findIcon = new ImageIcon(getClass.getResource("/org/aiotrade/platform/modules/quicksearch/resources/find.png")) // NOI18N
  ...
  private val animationTimer = new Timer(100, new ActionListener {
      lazy val icons = {
        ...
        for (i <- 0 until 8) {
          x(i) = ImageUtilities.loadImageIcon("/org/aiotrade/platform/modules/quicksearch/resources/progress_" + i + ".png", false)  //NOI18N
        }
        x
      }
