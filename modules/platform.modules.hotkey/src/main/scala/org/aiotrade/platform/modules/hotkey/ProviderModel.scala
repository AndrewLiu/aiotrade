...
object ProviderModel {
  ...
  private def loadCategories: List[Category] = {
    ...
    val sortedCats = FileUtil.getOrder(jList, false)
    ...
    while (itr.hasNext) {
      ...
      try {
        displayName = curFO.getFileSystem.getStatus.annotateName(
          ...
          Logger.getLogger(classOf[ProviderModel].getName).log(Level.WARNING,
      ...
      categories += (new Category(curFO, displayName, commandPrefix))
    }

    categories.toList
  }

  class Category(fo: FileObject, val displayName: String, val commandPrefix: String) {
    lazy val providers = {
